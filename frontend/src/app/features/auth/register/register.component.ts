      ...
      <section class="relative z-10 w-full max-w-md">
        ...
        <div class="rounded-2xl border border-slate-200 dark:border-[#374151] bg-white dark:bg-[#111827] shadow-xl p-7 md:p-8">
          <div class="mb-6">
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-4">Build automated trading journeys for stocks, forex, and derivatives from one platform.</p>
            <p class="text-xs text-slate-400 dark:text-slate-500 mt-2">Set strategy rules, manage risk, and execute with market-aware automation.</p>
          </div>

          <h2 class="text-2xl font-bold mb-1">Register your account</h2>
          ...
          <form (ngSubmit)="onSubmit()" class="space-y-4">
            <label for="name" class="block text-xs text-slate-500 dark:text-slate-400">
              Full name
              <input
                id="name"
                type="text"
          ...
          <p class="mt-5 text-sm text-center text-slate-500 dark:text-slate-400">
            ...
            <a routerLink="/auth/login" class="text-[#2563EB] font-medium">Log in</a>
          </p>
        </div>
      </section>
    </div>
...
export class RegisterComponent {
  ...
  onSubmit(): void {
    this.error = '';
    const result = this.auth.register(this.email, this.password, this.name);
    if (result.success) {
